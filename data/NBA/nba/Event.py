...
class Event:
    ...
    def __init__(self, event):
        ...
        self.player_team_dict = dict()        
        player_ids = [player['playerid'] for player in players]
        
        for player in home_players:
            self.player_team_dict[player['playerid']] = 0
        for player in guest_players:
            self.player_team_dict[player['playerid']] = 1
    ...
    def get_traj(self):
        ...
        all_all_player_id_teams = [] #(N,15,11,1)
        
        for i in range(traj_num):
            all_player_locations = [] # (15,11,2)
            all_player_id_teams = [] # (15,11)
            ...
            for j in range(15):
                ...
                for k in range(10):
                    ...
                    player_id_teams.append(self.player_team_dict[cur_moment.players[k].id])
                # player_locations.append([cur_moment.ball.x,cur_moment.ball.y])
                # player_id_teams.append(2)
                all_player_locations.append(player_locations)
                all_player_id_teams.append(player_id_teams)
                
            all_all_player_locations.append(all_player_locations)
            all_all_player_id_teams.append(all_player_id_teams)
        all_all_player_locations = np.array(all_all_player_locations,dtype=np.float32)
        all_all_player_id_teams = np.array(all_all_player_id_teams,dtype=np.int)
        del_list = []
        ...
        all_all_player_id_teams = np.delete(all_all_player_id_teams,del_list,axis=0)
        ...
        all_all_player_locations = np.delete(all_all_player_locations,del_list,axis=0)
        all_all_player_id_teams = np.delete(all_all_player_id_teams,del_list,axis=0)
        return all_all_player_locations,all_all_player_id_teams
