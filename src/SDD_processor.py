...
class Trajectory_SDD_Dataloader():
    def __init__(self,args):
        self.args = args
        self.skip = 12  # 1s30帧，间隔12帧取一注释帧 相应的 skip=1/30*12 = 0.4s 符合8帧预测12帧 3.2s预测4.8s
        # 处理数据保存的地址
        self.train_data_file = os.path.join(self.args.save_dir,'train_trajectroies.cpkl')
        self.test_data_file = os.path.join(self.args.save_dir,'test_trajectroies.cpkl')
        ...
        # -----------验证SDD在STAR数据集上的初始效果  不需要更改添加 label，scene等数据信息----------------
        ...
        print("creating pre_processed data from raw data")
